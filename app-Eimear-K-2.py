...
def load_workbooks():
    ...
    db = get_db("app.db")
    workbooks = {}
    rows = db.execute("SELECT title, questions_file, solutions_file, db_file, preamble FROM workbooks").fetchall()
    for row in rows:
        title = row["title"]
...
def get_workbook(workbook_id):
    ...
    workbook = user_db.execute(
        '''SELECT title, questions_file, solutions_file, db_file, preamble FROM workbooks WHERE id = ?''', 
        (workbook_id,)
    ).fetchone()
...
def start(): 
    return render_template("start.html", subtitle = "SQL Learning Environment") 
...
#            return render_template("/county.html", form=form, title="Guess the county")
    ...
    return render_template("/login.html", form=form, subtitle="Login") 
...
#            return render_template("/county.html", form=form, title="Guess the county")
    ...
    return render_template("/teacher.html", form=form, subtitle="Teacher Login")  
...
def upload(): 
    ...
    form.existing_db.choices = [(db, db) for db in db_files]
    ...
    if form.validate_on_submit():
        try:
            ...
            if allowed_file(question_file_data.filename) and allowed_file(solution_file_data.filename):
                ...
                if selected_db:
                    d_file_path = os.path.join(db_folder, selected_db)  # Use existing DB
                ...
                db = get_db("app.db")
                db.execute(
                    "INSERT INTO workbooks (title, questions_file, solutions_file, db_file, preamble) VALUES (?, ?, ?, ?, ?)",
                    (title, q_file_path, s_file_path, d_file_path, preamble),
                )
                db.commit()
    ...
    return render_template("upload.html", form=form, message=message, subtitle="Upload")
...
#         return render_template("/index.html", form=form, title="SQL", result=result  )
